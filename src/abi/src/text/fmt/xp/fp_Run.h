...
public:
  ...
  fp_InkRun *         getInk() const 			      { return m_pInk;} // KMC: ink
#if DEBUG
  ...
  void					setInk(fp_InkRun * pI); // KMC: ink
	...
	virtual bool	isDirty(void) const { return m_bDirty; }
  ...
  void          _drawTextLine(UT_sint32, UT_sint32, UT_uint32, UT_uint32, UT_UCSChar *);
	...
	virtual bool	findNextInkSplitPoint(fp_RunSplitInfo& /*si*/) { return false; }
...
protected:
  ...
  void          _setInk(fp_InkRun * pI) { m_pInk = pI; } //KMC: Ink
...
//START KMC: INK
class ABI_EXPORT fp_InkRun : public fp_Run
{
public:
	fp_InkRun(fl_BlockLayout* pBL, GR_Graphics* pG, UT_uint32 iOffsetFirst, UT_uint32 iLen);
	~fp_InkRun();
	bool 				isStartOfInk() const {return m_bIsStart;};
	const XML_Char * 	getInk() const {return (const XML_Char *)m_pInk;};
  ...
  // try making this permanently dirty (or is that what is causing our loop?)
  // KMC: no, doesn't help virtual bool	isDirty(void) const { return true; }
