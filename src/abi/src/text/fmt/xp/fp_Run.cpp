...
void fp_Run::insertIntoRunListBeforeThis(fp_Run& newRun)
{
	...
	if (m_pPrev)
	{
		...
		// KMC: seems to link current run to existing hyperlink (inside a hyperlink)
    if(newRun.getType()!= FPRUN_HYPERLINK) {
		  newRun.setHyperlink( m_pPrev->getHyperlink());
    }
    // KMC: so let's dop the same
    if(newRun.getType()!= FPRUN_INK) {
		  newRun.setInk( m_pPrev->getInk());
    }
	}
...
void fp_Run::insertIntoRunListAfterThis(fp_Run& newRun)
{
  ...
  // seems to link into existing hyperlink (surrounding)
  if(newRun.getType()!= FPRUN_HYPERLINK) {
		newRun.setHyperlink(m_pHyperlink);
  }
  // KMC: so let's dop the same
  if(newRun.getType()!= FPRUN_INK) {
		newRun.setInk(m_pInk);
  }
...
void fp_Run::unlinkFromRunList()
{
  ...
  // KMC: just mimicing the hyperlink run (see above)
  // KMC: should just remove all references to this ink from subsequent runs
  // KMC_TODO good for now, unless we decide that each run has it's own unique ink
	if(getType() == FPRUN_INK)
	{
		fp_InkRun * pI = static_cast<fp_InkRun*>(this);
		if(pI->isStartOfInk())
		{
			fp_Run * pRun = getNext();

			while(pRun && pRun->getInk() == pI)
			{
				pRun->setInk(NULL);
				pRun = pRun->getNext();
			}
...
// KMC: just mimicing setHyperlink (above)
void	fp_Run::setInk(fp_InkRun * pI)
{
	if(pI != m_pInk)
	{
		_setInk(pI);
		//KMC 7/25/03 3:56PM maybe this is causing our loop
    clearScreen();
	}
}
...
fp_InkRun::fp_InkRun( fl_BlockLayout* pBL,
								GR_Graphics* pG,
								UT_uint32 iOffsetFirst,
								UT_uint32 /*iLen*/)
	: fp_Run(pBL, pG, iOffsetFirst, 1, FPRUN_INK)
{
	_setLength(1);
  //KMC 10/13/2003 setLength(1);
	_setDirty(true);
	...
	while(pAP->getNthAttribute(k++, pName, pInk))
	{
		bFound = (0 == UT_XML_strnicmp(pName,"stroke",6));
		if(bFound)
			break;
	}
	...
	if(bFound)
	{
		UT_uint32 iTargetLen = UT_XML_strlen(pInk);
		m_pInk = new XML_Char [iTargetLen + 1];
		UT_XML_strncpy(m_pInk, iTargetLen + 1, pInk);
		m_bIsStart = true;
		//if this is a start of the hyperlink, we set m_pHyperlink to this,
		//so that when a run gets inserted after this one, its m_pHyperlink is
		//set correctly
...
fp_InkRun::~fp_InkRun()
{
  // clean up string containing persisted ink
  if(m_pInk) {
		delete [] m_pInk;
  }
}

void fp_InkRun::_lookupProperties(const PP_AttrProp * /*pSpanAP*/,
									   const PP_AttrProp * /*pBlockAP*/,
									   const PP_AttrProp * /*pSectionAP*/)
{
  return;
...
bool fp_InkRun::canBreakAfter(void) const
{  
  // KMC: changed to true just for fun 7/18/2003
	//return true;
  return false;
}

bool fp_InkRun::canBreakBefore(void) const
{
  // KMC: changed to true just for fun 7/18/2003
	//return true;
  return false;
...
bool fp_InkRun::letPointPass(void) const
{
	return true;
}

void fp_InkRun::mapXYToPosition(UT_sint32 x, UT_sint32 y, PT_DocPosition& pos, bool& bBOL, bool& bEOL)
{
	fp_Run *pRun = getNext();
	UT_ASSERT(pRun);
	pRun->mapXYToPosition(x, y, pos, bBOL, bEOL);
...
void fp_InkRun::findPointCoords(UT_uint32 iOffset, UT_sint32& x, UT_sint32& y,  UT_sint32& x2, UT_sint32& y2, UT_sint32& height, bool& bDirection)
{
  ...
  // from ImageRun
  /*
  UT_DEBUGMSG(("fintPointCoords: InkRun\n"));
	UT_sint32 xoff;
	UT_sint32 yoff;
...
void fp_InkRun::_clearScreen(bool /* bFullLineHeightRect */)
{  
	UT_ASSERT(!isDirty());

	UT_ASSERT(getGR()->queryProperties(GR_Graphics::DGP_SCREEN));
	
	// xoff and yoff should probably be the lower right hand corner
	//UT_sint32 xoff = 0, yoff = 0;

	// get the width and height of the rectangle to clear
	//UT_sint32 width = 0, height = 0;

	// need to clear full height of line, in case we had a selection
	//getLine()->getScreenOffsets(this, xoff, yoff);
	//UT_sint32 iLineHeight = getLine()->getHeight();
	
	//getGR()->fillRect(_getColorPG(),xoff, yoff, width, height);
 
  FV_View *ptrView = this->_getView();

  UT_ASSERT(ptrView);

  XAP_Frame *pXFrame = (XAP_Frame *)ptrView->getParentData();
  AP_Win32Frame *pFrame = (AP_Win32Frame *)(pXFrame);

  if(pFrame) {
    UT_ASSERT(pFrame);

    AP_Win32FrameImpl *pFrameImpl = (AP_Win32FrameImpl *)(pFrame->getFrameImpl());
  
    if (pFrameImpl) {
      UT_ASSERT(pFrameImpl);

      // KMC: redisplay the ink (we really will need to perform some kind of translation before
      // passing it back up to the ink collector, but for now, let's just ignore that fact.
      const XML_Char *strInk = this->getInk();

      // get it going
      // pFrameImpl->getInkOverlay()->resizeInkAreaToDocument(ptrView);

      if(strInk) {
        if(strlen(strInk) > 0) {
					ScreenRect box;

					// get the stroke's rectange/bounding box
					box = pFrameImpl->getInkOverlay()->getScreenRectangle(getGR(), strInk);

					// clear the screen
					getGR()->fillRect(_getColorPG(),box.left, box.top, box.width, box.height);
        }
      }
    }
  }
...
void fp_InkRun::_draw(dg_DrawArgs* pDA)
{  
	UT_DEBUGMSG(("KMC: fp_InkRun::_draw\n"));
	...
	// KMC: based on my observations, my guess is that these
	// x,y coords are the correct position to move the ink to
	// on the screen (now to get the ink to MOVE to this location)
	/*
	UT_DEBUGMSG(("xoff %i\n", xoff));
	UT_DEBUGMSG(("yoff %i\n", yoff));
	UT_DEBUGMSG(("pDA->xoff %i\n", pDA->xoff));
	UT_DEBUGMSG(("pDA->yoff %i\n", pDA->yoff));
	*/
	...
	UT_DEBUGMSG(("getX %i\n", this->getX()));
	UT_DEBUGMSG(("getY %i\n", this->getY()));
	UT_DEBUGMSG(("getWindowHeight %i\n", ptrView->getWindowHeight()));
	UT_DEBUGMSG(("getWindowWidth %i\n", ptrView->getWindowWidth()));
	UT_DEBUGMSG(("getXScrollOffset %i\n", ptrView->getXScrollOffset()));
	UT_DEBUGMSG(("getYScrollOffset %i\n", ptrView->getYScrollOffset()));
	UT_DEBUGMSG(("getDocPositionFromXY %i\n", ptrView->getDocPositionFromXY(pDA->xoff, pDA->yoff)));
	UT_DEBUGMSG(("point %i\n", ptrView->getPoint()));
	*/
	...
	*/
	...
	GR_Win32Graphics *pGR = (GR_Win32Graphics *)pDA->pG;
	GR_Win32InkOverlay *inkOverlay = pGR->getInkOverlay();

	if(strInk) {
		if(strlen(strInk) > 0) {
			// clear the screen first
			ScreenRect box;
			box = inkOverlay->getScreenRectangle(pGR, strInk);
			//getGR()->fillRect(_getColorPG(),box.left, box.top, box.width, box.height);					
			//int inkLength = strlen(strInk);
			//char lastChar = strInk[inkLength - 1];

			// but what about the transformations man?!
			// WE SHOULD BE ABLE TO PASS IN THE SCREEN COORDINATES FROM HERE!!!!!
			// KMC 8/11/2003: pDA has the coords you dummy!
			//pFrameImpl->getInkOverlay()->drawInk(pDA, strInk);          
			bool bolReflowed = false;
			bolReflowed = inkOverlay->drawStroke(pDA, strInk);

			// if the ink reflowed, then we need to refresh that area of the screen
			// since the old ink mark is there. if it didn't reflow, then this was just
			// a simple window redraw request rather than a user change request
			if(bolReflowed) {
				// refreshes everything in the old box
				RECT rect;
				rect.top = box.top;
				rect.left = box.left;
				rect.bottom = box.top + box.height;
				rect.right = box.left + box.width;					
				RedrawWindow(pGR->getHwnd(), &rect, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_ERASENOW); 
			}
