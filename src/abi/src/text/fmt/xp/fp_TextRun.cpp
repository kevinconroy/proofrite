...
/*
 KMC 2/16/04
 Determines the next word break to use for inserting a new ink run (non-hyphenable ink)
 Based on findMaxLeftFitSplitPoint (see function directly above)
 \retval si Split information (left width, right width, and position)
 \return True if split point was found in this Run, otherwise false.
*/
bool fp_TextRun::findNextInkSplitPoint(fp_RunSplitInfo& si)
{
	// get the char widths of the text in this run
#if !defined(WITH_PANGO) && defined(USE_LAYOUT_UNITS)
	UT_GrowBuf * pgbCharWidths = getBlock()->getCharWidths()->getCharWidthsLayoutUnits();
	UT_sint32 iRightWidth = getWidthInLayoutUnits();
#else
	...
	// get the internal char width buffer
	UT_GrowBufElement* pCharWidths = pgbCharWidths->getPointer(0);

	// if we don't have char widths, bail since there aren't any words
	if(pCharWidths == NULL)
	{
		return false;
	}

	// there are words... figure it out
	UT_sint32 iLeftWidth = 0;
	...
	while (bContinue)
	{
		...
		// this is left over from adopted code source (findMaxLeftFitSplitPoint)
		if(!bContinue)
		{
			// this block has got a text run but no span in the PT,
			// this is clearly a bug
		...
		// and bContinue since we're trying to find the max left break (rather than max right!)
		for (UT_uint32 i=0; i<lenSpan && bContinue; i++)
		{
			UT_sint32 iCW = pCharWidths[i + offset] > 0 ? pCharWidths[i + offset] : 0;
			iLeftWidth += iCW;
			iRightWidth -= iCW;

			if ( XAP_EncodingManager::get_instance()->can_break_at(pSpan[i]) || ((UCS_SPACE == pSpan[i])))
			//		&& ((i + offset) != (getBlockOffset() + getLength() - 1))))
			{				
				si.iLeftWidth = iLeftWidth;
				si.iRightWidth = iRightWidth;
				...
				// found a space or another valid break point - halt
				bContinue = false;
			}
		}
...
bool fp_TextRun::split(UT_uint32 iSplitOffset)
{
  ...
  // KMC: suckers; may need to split ink here at some point
  // TODO split ink?!
  pNew->_setInk(this->getInk());
