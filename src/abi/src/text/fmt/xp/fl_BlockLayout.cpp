...
// KMC: mimics the _doInsertHyperlink function and will link subsequent runs to this ink run
// KMC_TODO (not sure if this is good or not yet)
bool	fl_BlockLayout::_doInsertInkRun(PT_BlockOffset blockOffset)
{
	fp_InkRun * pNewRun =  new fp_InkRun(this, m_pLayout->getGraphics(), blockOffset, 1);
	UT_ASSERT(pNewRun);
	bool bResult = _doInsertRun((fp_Run*)pNewRun);
	if (bResult)
	{
		...
		if(pNewRun->isStartOfInk())
		{
			fp_Run * pRun = pNewRun->getNext();
			UT_ASSERT(pRun);
			// when loading a document the opening hyperlink run is initially followed
			// by ENDOFPARAGRAPH run; we do not want to set this one
			while(pRun && pRun->getType() != FPRUN_INK && pRun->getType() != FPRUN_ENDOFPARAGRAPH)
			{
				pRun->setInk(pNewRun);
				pRun = pRun->getNext();
			}
...
//        what is in the piecetable
		...
		// Verify that the Run has a non-zero length (or is a FmtMark or Ink)
		UT_ASSERT( (FPRUN_FMTMARK == pRun->getType() 
				   || FPRUN_INK == pRun->getType())
				   || (pRun->getLength() > 0) );
												...
												const PX_ChangeRecord_Object * pcro)
{
	...
	case PTO_Ink:
		UT_DEBUGMSG(("Populate:InsertInk:\n"));
		_doInsertInkRun(blockOffset);
		return true;
...
bool fl_BlockLayout::doclistener_insertObject(const PX_ChangeRecord_Object * pcro)
{
  ...
  case PTO_Ink:
	{
		UT_DEBUGMSG(("Edit:InsertObject:Ink:\n"));
		blockOffset = pcro->getBlockOffset();
    _doInsertInkRun(blockOffset);
		break;
...
bool fl_BlockLayout::doclistener_deleteObject(const PX_ChangeRecord_Object * pcro)
{
	...
	switch (pcro->getObjectType())
	{
		...
		case PTO_Ink:
		{
			UT_DEBUGMSG(("Edit:DeleteObject:Ink:\n"));
			blockOffset = pcro->getBlockOffset();
			_delete(blockOffset,1);
			break;
		}
