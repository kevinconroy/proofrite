									 ...
									 pf_Frag_Object ** ppfo)
{
    ...
    case PTO_Ink:
      {
        // KMC: i think we can handle this at other levels and treat an image like a mix
        //   between a bookmark and a hyperlink
        UT_DEBUGMSG(("KMC: pt_PT_InsertObject: should we do anything special for ink? Don't think so\n")); // drop thru
      }
    case PTO_Hyperlink:
		case PTO_Image:
		case PTO_Field:
                                     ...
                                     pf_Frag_Object * & pfo)
{
	...
	// KMC: added hack to fix EoD ink insertion bug
	if (fragOffset == 0) // || pf->getType() == pf_Frag::PFT_EndOfDoc)
	{
