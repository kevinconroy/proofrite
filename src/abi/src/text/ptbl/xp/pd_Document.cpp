...
// KMC: need this since we're updating ink!
bool PD_Document::updateDataItem(const char * szName, bool bBase64, const UT_ByteBuf * pByteBuf,
									void* pToken,
									void ** ppHandle)
{
	...
	if (getDataItemDataByName(szName,NULL,NULL,NULL) == true) {
		...
		struct _dataItemPair* pPair = (struct _dataItemPair*) m_hashDataItems.pick(szName);
		UT_ASSERT(pPair);
		m_hashDataItems.remove (szName, NULL);
		delete pPair->pBuf;
		FREEP(pPair->pToken);
		delete pPair;
	}
	UT_ASSERT(!getDataItemDataByName(szName,NULL,NULL,NULL));

  return createDataItem(szName, bBase64, pByteBuf, pToken, ppHandle);
}
...
#if TABIPADD
// returns a copy of the current doc's id
DocID_t	PD_Document::getPADDDocId() {
	DocID_t value(this->m_docId);
	
	return value;
}

// determines if a doc id is present or not
bool PD_Document::hasPADDDocId() {
	bool value = false;

	if(this->m_docId.length() > 0) {
		value = true;
	}
...
// sets the doc id for the current document (overwrites any existing value)
void PD_Document::setPADDDocId(DocID_t docId) {
	// will use operator= to copy
	this->m_docId = docId;
}
#endif
