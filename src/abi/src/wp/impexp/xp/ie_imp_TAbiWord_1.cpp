...
/* By: Kevin Conroy
 * Comments: This file is merely an extension of the AbiWord_1 importer; I really should move this
...
IE_Imp_TAbiWord_1_Sniffer::IE_Imp_TAbiWord_1_Sniffer ()
	: IE_ImpSniffer(IE_IMPEXPNAME_AWML11)
{
	// 
}

UT_Confidence_t IE_Imp_TAbiWord_1_Sniffer::supportsMIME (const char * szMIME)
{
...
UT_Confidence_t IE_Imp_TAbiWord_1_Sniffer::recognizeContents (const char * szBuf,
												  UT_uint32 iNumbytes)
{
	...
	while( iLinesToRead-- )
	{
		...
		magic = "<!-- This file is an TAbiWord document." ;
		if ( (iNumbytes - iBytesScanned) < strlen(magic) ) return(UT_CONFIDENCE_ZILCH);
		if ( strncmp(p, magic, strlen(magic)) == 0 ) return(UT_CONFIDENCE_PERFECT);
...
UT_Confidence_t IE_Imp_TAbiWord_1_Sniffer::recognizeSuffix (const char * szSuffix)
{
  if (!UT_stricmp(szSuffix, ".tabw") || !UT_stricmp(szSuffix, ".tawt"))
    return UT_CONFIDENCE_PERFECT;
  return UT_CONFIDENCE_ZILCH;
}

bool IE_Imp_TAbiWord_1_Sniffer::getDlgLabels (const char ** szDesc,
											 const char ** szSuffixList,
											 IEFileType * ft)
{
	*szDesc = "TAbiWord Documents (.tabw, .tawt)";
	*szSuffixList = "*.tabw; *.tawt";
	*ft = getFileType();
	return true;
}

UT_Error IE_Imp_TAbiWord_1_Sniffer::constructImporter (PD_Document * pDocument,
													  IE_Imp ** ppie)
{
	IE_Imp_TAbiWord_1 * p = new IE_Imp_TAbiWord_1(pDocument);
	*ppie = p;
	return UT_OK;
}
...
IE_Imp_TAbiWord_1::~IE_Imp_TAbiWord_1()
{
  if ( !m_bWroteSection )
...
IE_Imp_TAbiWord_1::IE_Imp_TAbiWord_1(PD_Document * pDocument)
...
UT_Error IE_Imp_TAbiWord_1::importFile(const char * szFilename)
{
...
static struct xmlToIdMapping s_Tokens[] =
{
  ...
  {	"tabiword",		TT_DOCUMENT		},
	{	"table",		TT_TABLE		}
};

#define TokenTableSize	((sizeof(s_Tokens)/sizeof(s_Tokens[0])))

void IE_Imp_TAbiWord_1::startElement(const XML_Char *name, const XML_Char **atts)
{
...
//
  ...
  case TT_INK:
		X_VerifyParseState(_PS_Block);
		X_CheckError(getDoc()->appendObject(PTO_Ink,atts));
		return;
...
void IE_Imp_TAbiWord_1::endElement(const XML_Char *name)
{
	...
	case TT_INK:						// not a container, so we don't pop stack
		UT_ASSERT_HARMLESS(m_lenCharDataSeen==0);
		X_VerifyParseState(_PS_Block);
		// we append another Ink Object, but with no attributes
    // KMC: I'm guessing that this is so that you can tell where the hyperlink/ink ends
    //   and this end tag would be used to generate the </a>/</ink> tag
		X_CheckError(getDoc()->appendObject(PTO_Ink,NULL));
		return;
		//return;
...
const XML_Char * IE_Imp_TAbiWord_1::_getDataItemName(const XML_Char ** atts)
{
	return _getXMLPropValue ("name", atts);
}

const XML_Char * IE_Imp_TAbiWord_1::_getDataItemMimeType(const XML_Char ** atts)
{
...
bool IE_Imp_TAbiWord_1::_getDataItemEncoded(const XML_Char ** atts)
{
...
bool IE_Imp_TAbiWord_1::_handleImage (const XML_Char ** atts)
{
#ifdef ENABLE_RESOURCE_MANAGER
...
bool IE_Imp_TAbiWord_1::_handleResource (const XML_Char ** atts, bool isResource)
{
#ifdef ENABLE_RESOURCE_MANAGER
	...
	if (isResource)
	...
	else
		{
			...
			// <d name="ID" mime-type="image/svg-xml | text/mathml | text/ink-xml" base64="no"> <![CDATA[ ... ]]> </d>

			const XML_Char * r_id = 0;
			const XML_Char * r_64 = 0;

			enum { mt_unknown, mt_png, mt_svg, mt_mathml, mt_ink } mt = mt_unknown;
