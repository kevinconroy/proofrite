...
/* By: Kevin Conroy
* Comments: This file is merely an extension of the AbiWord_1 exporter; I really should move this
*  to inherit from this class an just add handlers for the ink, but for easy of start up, I'm just going
*  to make it a seperate class for now and may try to merge it into a derived version of AbiWord_1
*  (yeah, right... if I only had a life time to do this project)
*/
...
IE_Exp_TAbiWord_1_Sniffer::IE_Exp_TAbiWord_1_Sniffer ()
: IE_ExpSniffer(IE_IMPEXPNAME_AWML11)
{
  // 
}

UT_Confidence_t IE_Exp_TAbiWord_1_Sniffer::supportsMIME (const char * szMIME)
{
...
bool IE_Exp_TAbiWord_1_Sniffer::recognizeSuffix(const char * szSuffix)
{
  return (!UT_stricmp(szSuffix,".tabw"));
}

UT_Error IE_Exp_TAbiWord_1_Sniffer::constructExporter(PD_Document * pDocument,
                                                      IE_Exp ** ppie)
{
  IE_Exp_TAbiWord_1 * p = new IE_Exp_TAbiWord_1(pDocument);
  *ppie = p;
  return UT_OK;
}

bool IE_Exp_TAbiWord_1_Sniffer::getDlgLabels(const char ** pszDesc,
                                             const char ** pszSuffixList,
                                             IEFileType * ft)
{
  *pszDesc = "TAbiWord (.tabw)";
  *pszSuffixList = "*.tabw";
  *ft = getFileType();
  return true;
}
...
IE_Exp_TAbiWord_1::IE_Exp_TAbiWord_1(PD_Document * pDocument, bool isTemplate)
: IE_Exp(pDocument), m_bIsTemplate(isTemplate), m_pListener(0)
{
  m_error = 0;
}

IE_Exp_TAbiWord_1::~IE_Exp_TAbiWord_1()
{
}
...
#ifdef ENABLE_RESOURCE_MANAGER
class s_TAbiWord_1_Listener : public PL_Listener, XAP_ResourceManager::Writer
#else
class s_TAbiWord_1_Listener : public PL_Listener
#endif
{
public:
  s_TAbiWord_1_Listener(PD_Document * pDocument,
    IE_Exp_TAbiWord_1 * pie, bool isTemplate);
  virtual ~s_TAbiWord_1_Listener();
...
void s_TAbiWord_1_Listener::_closeSection(void)
{
  if (!m_bInSection)
    return;
...
void s_TAbiWord_1_Listener::_closeTable(void)
{
  if (m_iInTable == 0)
    return;
...
void s_TAbiWord_1_Listener::_closeCell(void)
{
  if (m_iInCell == 0)
    return;
...
void s_TAbiWord_1_Listener::_closeBlock(void)
{
  if (!m_bInBlock)
    return;
...
void s_TAbiWord_1_Listener::_closeSpan(void)
{
  if (!m_bInSpan)
    return;
...
void s_TAbiWord_1_Listener::_closeTag(void)
{
  if (m_bOpenChar) m_pie->write("</c>");
  m_bOpenChar = false;
}

void s_TAbiWord_1_Listener::_closeField(void)
{
  if (!m_pCurrentField)
    return;
  _closeSpan();
...
void s_TAbiWord_1_Listener::_closeHyperlink(void)
{
  if (!m_bInHyperlink) {
    return;
  }
...
// KMC: Ink
void s_TAbiWord_1_Listener::_closeInk(void)
{
  if (!m_bInInk) {
    return;
  }
...
void s_TAbiWord_1_Listener::_openSpan(PT_AttrPropIndex apiSpan)
{
  if (m_bInSpan)
  {
...
void s_TAbiWord_1_Listener::_openTag(const char * szPrefix, const char * szSuffix,
                                     bool bNewLineAfter, PT_AttrPropIndex api,
...
void s_TAbiWord_1_Listener::_outputXMLChar(const XML_Char * data, UT_uint32 length)
{
  UT_String sBuf;
...
void s_TAbiWord_1_Listener::_outputData(const UT_UCSChar * data, UT_uint32 length)
{
  UT_UTF8String sBuf;
...
s_TAbiWord_1_Listener::s_TAbiWord_1_Listener(PD_Document * pDocument,
                                             IE_Exp_TAbiWord_1 * pie,
                                             bool isTemplate)
                                             : m_pUsedImages(ut_lexico_lesser)
{
  ...
  ************************************************************************************/
  ...
  _openTag("tabiword", NULL, true, pDocument->getAttrPropIndex(),false);

  // NOTE we output the following preamble in XML comments.
  // NOTE this information is for human viewing only.

  m_pie->write("<!-- =====================================================================  -->\n");
  m_pie->write("<!-- This file is a TAbiWord document.                                      -->\n");
  m_pie->write("<!-- TAbiWord is a free, Open Source word processor based on AbiWord.       -->\n");
  m_pie->write("<!-- More information about TAbiWord is availabe at http://tabiword.sourceforge.net -->\n");
...
s_TAbiWord_1_Listener::~s_TAbiWord_1_Listener()
{
  _closeSpan();
  _closeField();
  _closeHyperlink();
  ...
  m_pie->write("</tabiword>\n");
}


const XML_Char*
s_TAbiWord_1_Listener::getObjectKey(const PT_AttrPropIndex& api, const XML_Char* key)
{
  const PP_AttrProp * pAP = NULL;
  bool bHaveProp = m_pDocument->getAttrProp(api,&pAP);
  if (bHaveProp && pAP)
...
bool s_TAbiWord_1_Listener::populate(PL_StruxFmtHandle /*sfh*/,
                                     const PX_ChangeRecord * pcr)
{
  switch (pcr->getType())
  {
...
bool s_TAbiWord_1_Listener::populateStrux(PL_StruxDocHandle /*sdh*/,
                                          const PX_ChangeRecord * pcr,
                                          PL_StruxFmtHandle * psfh)
{
...
bool s_TAbiWord_1_Listener::change(PL_StruxFmtHandle /*sfh*/,
                                   const PX_ChangeRecord * /*pcr*/)
{
  UT_ASSERT_NOT_REACHED();
  return false;
...
bool s_TAbiWord_1_Listener::insertStrux(PL_StruxFmtHandle /*sfh*/,
                                        const PX_ChangeRecord * /*pcr*/,
                                        PL_StruxDocHandle /*sdh*/,
                                        PL_ListenerId /* lid */,
                                        void (* /*pfnBindHandles*/)(PL_StruxDocHandle /* sdhNew */,
...
bool s_TAbiWord_1_Listener::signal(UT_uint32 /* iSignal */)
{
  UT_ASSERT_NOT_REACHED();
  return false;
}
...
*/
UT_Error s_TAbiWord_1_Listener::write_base64 (void * /*context*/, const char * base64, UT_uint32 length, bool /*final*/)
{
...
*/
UT_Error s_TAbiWord_1_Listener::write_xml (void * /*context*/, const char * name, const char * const * atts)
{
  UT_UTF8String tag(" <");
...
/* end tag
*/
UT_Error s_TAbiWord_1_Listener::write_xml (void * /*context*/, const char * name)
{
  UT_UTF8String tag(" </");
...
UT_Error IE_Exp_TAbiWord_1::_writeDocument(void)
{
  m_pListener = new s_TAbiWord_1_Listener(getDoc(),this, m_bIsTemplate);
  if (!m_pListener)
...
void s_TAbiWord_1_Listener::_handleStyles(void)
{
...
void s_TAbiWord_1_Listener::_handleIgnoredWords(void)
{
...
void s_TAbiWord_1_Listener::_handleLists(void)
{
...
void s_TAbiWord_1_Listener::_handleMetaData(void)
{
...
void s_TAbiWord_1_Listener::_handlePageSize(void)
{
  //
...
void s_TAbiWord_1_Listener::_handleDataItems(void)
{
#ifdef ENABLE_RESOURCE_MANAGER
...
void s_TAbiWord_1_Listener::_handleRevisions(void)
{
