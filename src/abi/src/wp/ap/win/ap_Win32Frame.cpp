...
{
  XAP_FrameImpl *pFImpl = this->getFrameImpl();
  AP_Win32FrameImpl *pFImplWin32 = (AP_Win32FrameImpl *)(pFImpl);

  // if it's a valid cast
  if(pFImplWin32 != 0) {
    // convert to a decimal (it's in a percentage right now)
    float zoomLevel = ((float)iZoom/100);

    // get the ink overlay
    GR_Win32InkOverlay *pInkOverlay = pFImplWin32->getInkOverlay();

    // set the zoom level 
    pInkOverlay->setZoomLevel(zoomLevel);
  }

	// show the document AFTER we've updated the zoom information
	// otherwise the ink is gets invalid/unzoomed coordinates for reflowing
	_showDocument(iZoom);
...
bool AP_Win32Frame::_createViewGraphics(GR_Graphics *& pG, UT_uint32 iZoom)
{
	...
	GR_Win32InkOverlay *pInkOverlay = static_cast<AP_Win32FrameImpl *>(getFrameImpl())->getInkOverlay();
	pG = new GR_Win32Graphics(GetDC(hwndDocument), hwndDocument, getApp(), pInkOverlay);
