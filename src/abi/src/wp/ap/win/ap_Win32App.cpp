...
#include <io.h>
#include <fcntl.h>
//include <iostream.h> // KMC: removed since wasn't linking on the CS machine
#include <ole2.h>
...
static LRESULT CALLBACK _SplashWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_PAINT:
    hdc = BeginPaint(hWnd, &ps);
    {
      ...
      GR_Graphics * pG = new GR_Win32Graphics(hdc, hwndSplash, 0, 0);
...
static GR_Image * _showSplash(HINSTANCE hInstance, const char * szAppName)
{
    ...
    )
  {
    ...
    if (hwndSplash) 
    {
      // create image first
      // TODO: find XAP_App pointer for this
      GR_Graphics * pG = new GR_Win32Graphics(GetDC(hwndSplash), hwndSplash, 0, 0);
...
{
  //INK: static CMyInkCollector *pmic = NULL;
  bool bShowApp = true;
  ...
  {	
    ...
    //INK: pMyWin32App->initializeTablet(); // KMC
  ...
  {		
    ...
    if (bShowApp)
    {
      ...
      // do dispatch loop
      // KMC: MAIN LOOP OF PROGRAM --  seems to grab all messages and pass them on
...
/*INK
bool AP_Win32App::initializeTablet() {
  // Allocate and initialize memory for object
  this->m_pmic= new CMyInkCollector();
  ...
  if (this->m_pmic != NULL)
  {
    ...
    //  the current window. 
    if (SUCCEEDED(this->m_pmic->Init(hWnd)))
    {
      return true;
    }
