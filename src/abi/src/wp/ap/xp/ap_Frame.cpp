...
ReplaceDocument:
	...
	((PD_Document *)pNewDoc)->getDataItemDataByName("ink",NULL,NULL,NULL);
			    ...
			    UT_uint32 iZoom)
{
	...
	// KMC: INK
	// need to deserialize the ink now since we're replacing the view
	// and want to make sure that the ink overlay has the correct ink
	GR_Win32Graphics *pGR = (GR_Win32Graphics *)pG;
	if(pGR) {

		const UT_ByteBuf *pByteBuf = 0;
		pDocLayout->getDocument()->getDataItemDataByName("ink",&pByteBuf,NULL,NULL);		

		// if we have an ink object in the data section, then deserialize it
		if(pByteBuf != 0) {
			// copy from the byte buffer to a char array
			char * xmlInk = new char[pByteBuf->getLength() + 1];			
			const UT_Byte *byteInk = pByteBuf->getPointer(0);			
			for(UT_uint32 i=0; i < pByteBuf->getLength();i++) {
				xmlInk[i] = (char)byteInk[i];
			}
			xmlInk[i] = '\0';

			// deserialize the ink for this view
			pGR->getInkOverlay()->deserializeInk(xmlInk);
		}
		// no ink for this document, let's clear the ink!
		else {
			// clear
			pGR->getInkOverlay()->clearScreen();
		}
	}
