...
Defun_EV_GetMenuItemState_Fn(ap_GetState_InkAnnotation)
{
	ABIWORD_VIEW;
	UT_ASSERT(pView);

	// get current graphics and cast to a windows graphics object
	GR_Graphics *pGR = pView->getGraphics();
	GR_Win32Graphics *pWin32GR = (GR_Win32Graphics *)pGR;

	// see if the cast worked or not
	if(pWin32GR) {
		// get the ink overlay
...
Defun_EV_GetMenuItemState_Fn(ap_GetState_InkColor)
{
	ABIWORD_VIEW;
	UT_ASSERT(pView);

	const EV_Menu_Action *pAction = pView->getApp()->getMenuActionSet()->getAction(id);
	...
	if(pWin32GR) {
		...
		//InkColor color = pInkOverlay->getColor();
		const char * methodName = pAction->getMethodName();

		// just interested in the suffix which is the color
		const char * methodColor = &methodName[strlen("inkColor")];

		// get the current color of ink
		const char * color = pInkOverlay->getColorName();
		
		// if we have an id and the color matches the current color ;)
		if(id && strcmp(methodColor, color) == 0) {
			return EV_MIS_Toggled;
		}
		else {
			return EV_MIS_ZERO;
...
Defun_EV_GetMenuItemState_Fn(ap_GetState_InkWidth)
{
	ABIWORD_VIEW;
	UT_ASSERT(pView);
	...
	if(pWin32GR) {
		...
		// just interested in the suffix which is the color
		const char * methodWidth = &methodName[strlen("inkWidth")];

		// get the current color of ink
		const char * width = pInkOverlay->getWidthName();
		
		// if we have an id and the color matches the current color ;)
		if(id && strcmp(methodWidth, width) == 0) {
			return EV_MIS_Toggled;
		}
		else {
			return EV_MIS_ZERO;
...
Defun_EV_GetMenuItemState_Fn(ap_GetState_PenType)
{
	ABIWORD_VIEW;
	UT_ASSERT(pView);
	...
	if(pWin32GR) {
		...
		// just interested in the suffix which is the color
		const char * methodType = &methodName[strlen("inkType")];

		// get the current color of ink
		const char * type = pInkOverlay->getPenTypeName();
		
		// if we have an id and the color matches the current color ;)
		if(id && strcmp(methodType, type) == 0) {
			return EV_MIS_Toggled;
		}
		else {
			return EV_MIS_ZERO;
