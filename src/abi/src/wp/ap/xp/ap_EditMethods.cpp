...
public:
	...
	// KMC: INK START
  static EV_EditMethod_Fn insertInk;
	static EV_EditMethod_Fn inkToggleAnnotation;

	static EV_EditMethod_Fn inkColorBlack;
	static EV_EditMethod_Fn inkColorBlue;
	static EV_EditMethod_Fn inkColorGreen;
	static EV_EditMethod_Fn inkColorOrange;
	static EV_EditMethod_Fn inkColorRed;
	static EV_EditMethod_Fn inkColorYellow;

  static EV_EditMethod_Fn inkTypeBallpoint;
	static EV_EditMethod_Fn inkTypeHighlighter;
	static EV_EditMethod_Fn inkTypeMarker;

	static EV_EditMethod_Fn inkWidthLarge;
	static EV_EditMethod_Fn inkWidthLargest;
	static EV_EditMethod_Fn inkWidthMedium;
	static EV_EditMethod_Fn inkWidthSmall;
	static EV_EditMethod_Fn inkWidthSmallest;
	// KMC: INK END
...
static EV_EditMethod s_arrayEditMethods[] =
{
	...
	// i
	// ink functions (keep alphabetically sorted... they use a
	// binary search for O(log n) access time and it won't work unless
	// they can assume that it's statically sorted by the programmer
	EV_EditMethod(NF(inkColorBlack), 0, ""),
	EV_EditMethod(NF(inkColorBlue), 0, ""),
	EV_EditMethod(NF(inkColorGreen), 0, ""),
	EV_EditMethod(NF(inkColorOrange), 0, ""),
	EV_EditMethod(NF(inkColorRed), 0, ""),
	EV_EditMethod(NF(inkColorYellow), 0, ""),
	EV_EditMethod(NF(inkToggleAnnotation), 0, ""),
	EV_EditMethod(NF(inkTypeBallpoint), 0, ""),
  EV_EditMethod(NF(inkTypeHighlighter), 0, ""),
  EV_EditMethod(NF(inkTypeMarker), 0, ""),
  EV_EditMethod(NF(inkWidthLarge), 0, ""),
	EV_EditMethod(NF(inkWidthLargest), 0, ""),
	EV_EditMethod(NF(inkWidthMedium), 0, ""),
	EV_EditMethod(NF(inkWidthSmall), 0, ""),
	EV_EditMethod(NF(inkWidthSmallest), 0, ""),
  ...
  EV_EditMethod(NF(insertInk),		0,	""), //KMC
...
* Function: insertInk
* Purpose: Inserts the ink (as specified as a base64 XML string), into the document
****************************************************************************/
Defun(insertInk)
{
	CHECK_FRAME;
	ABIWORD_VIEW;    
	pView->cmdInsertInk(pCallData->m_xPos, pCallData->m_yPos, pCallData->m_pStroke);	
	return true;
}
...
/* All PADD stuff here */
bool s_printPageToPADD(PD_Document *doc, const PageAddr page_addr)
{
	PADDStatus_t ret = 0;
	
	// DML: "First thing's first"
	// Homestar: "Oh weally?"
	
	// remote database
	// TODO change hardcoded to preferences
	PADDClient paddClient("francois", "bristol.cs.umd.edu", 18322);

	// new string for document id
	DocID_t docId;
	
	// if doesn't have page id, get one
	if(!doc->hasPADDDocId()) {
		// temp name
		string fileName = "temp.tabi";
		
		// get the real file name if there is one
		if(doc->getFilename()) {
			fileName = doc->getFilename();
		}

		// debug
		UT_DEBUGMSG(("page_addr=[%s] fileName=[%s] docId=[%s]\n", page_addr.toString(), fileName, docId));

		// DML: get a new doc id
		ret = paddClient.get_docid(page_addr, TABIWORD_PROTO, fileName, &docId);
		...
		// died
		if(ret == PADD_FAIL) {
			UT_DEBUGMSG(("PADD ERROR: Unable to get a new document id because [%s]\n", paddClient.geterr()));
			return false;
		}
		// debug
		UT_DEBUGMSG(("New doc_id obtained: docId=[%s]\n", docId));

		// DML: want to put doc id into document
		doc->setPADDDocId(docId);
		
		// DML: get temp file name (from PrinterInterface.cpp)
		char *file = "c:";
		string path;
		/*
		char path[256];
		DWORD len = GetTempPath(256, path);
		char lfn[280];
		char* file, *tmp=fname_buf;
		while(tmp = strchr(tmp, '\\'))
		{
			file = tmp+1;
			tmp++;
		}		
		sprintf(lfn, "%s%s", path, file);
		*/		
		path.append(file);
		path.append("\\");
		...
		// debug
		UT_DEBUGMSG(("saving file to [%s]\n", path));

		// DML: and save document to a temporary file	
		doc->saveAs(path.c_str(), 1);

		// debug
		UT_DEBUGMSG(("giving doucment to PADD subsystem\n"));

		// DML: and upload the document to the file server
		ret = paddClient.put_document(file, docId);
		
		if(ret == PADD_FAIL) {
			UT_DEBUGMSG(("PADD ERROR: Unable to upload file [%s] because [%s]!!\n", file, paddClient.geterr()));			
			return false;
		}
	}
	// get existing document id
	else {
		docId = doc->getPADDDocId();
	}
...
#if TABIPADD
					// KMC & DML: PADD
					const PageAddr page_addr("22.14.33.120");
					s_printPageToPADD(doc, page_addr);
#endif
...
//
		...
		// get the existing ink overlay
		GR_Win32InkOverlay *inkOverlay = ((GR_Win32Graphics *)pView->getGraphics())->getInkOverlay();
		...
		GR_Graphics * pGraphics = pDialog->getPrinterGraphicsContext(inkOverlay);
...
Defun(inkToggleAnnotation)
{
	...
	if(pG)
	{
		...
		if(pWin32GR) {
			GR_Win32InkOverlay *pInkOverlay = pWin32GR->getInkOverlay();

			// if it's enabled, then disable it
			if(pInkOverlay->isEnabled()) {
				pInkOverlay->disableInkOverlay();
			}
			else {
				// otherwise re-enable it
				pInkOverlay->enableInkOverlay();
			}
		}
	}
...
Defun(inkTypeBallpoint)
{
	...
	if(pG)
	{
		...
		if(pWin32GR) {
			...
			pInkOverlay->setPenType(Ballpoint);
		}
	}
...
Defun(inkTypeHighlighter)
{
	...
	if(pG)
	{
		...
		if(pWin32GR) {
			...
			pInkOverlay->setPenType(Highlighter);
		}
	}
