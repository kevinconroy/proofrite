...
//
// Description:
//      The header file for InkOverlayEvents, which is designed
//      to be used by C++ applications as a base class for handling
//      Ink Overlay events. This base class is meant to be used
...
/////////////////////////////////////////////////////////
//                                          
// class InkOverlayEvents
// 
// The InkOverlayEvents class handles passing Ink Overlay
// events from the Ink Overlay to the user of this class.
// It sets up the connection between the InkOverlay and
// this class in AdviseInkOverlay. In the Invoke method,
...
// IInkOverlay::SetEventInterest to guarantee getting
...
// with the Ink Overlay.
//
/////////////////////////////////////////////////////////
class ABI_EXPORT InkOverlayEvents : public _IInkOverlayEvents
{
public:
		...
		void **ppvObject)
	{
		...
		// This object only supports IDispatch/_IInkOverlayEvents
		if ((riid == IID_IUnknown)
			|| (riid == IID_IDispatch)
			|| (riid == DIID__IInkOverlayEvents))
		{
	...
	HRESULT AdviseInkOverlay(
		IInkOverlay *pIInkOverlay)
	{
		HRESULT hr = S_OK;

		// Check to ensure that the sink is not currently connected
		// with another Ink Overlay...
		if (NULL == m_pIConnectionPoint)
		{
			// Get the connection point container
			IConnectionPointContainer *pIConnectionPointContainer;
			hr = pIInkOverlay->QueryInterface(
			...
			// Find the connection point for Ink Overlay events
			hr = pIConnectionPointContainer->FindConnectionPoint(
				__uuidof(_IInkOverlayEvents), &m_pIConnectionPoint);
		...
		// If the sink is already connected to an Ink Overlay, return a 
		// failure; only one Ink Overlay can be attached at any given time.
		else
		{
			hr = E_FAIL;
		}
	...
	// Remove the connection of the sink to the Ink Overlay
	HRESULT UnadviseInkOverlay()
	{
		HRESULT hr = S_OK;

		// If there the ink Overlay is connected to the sink,
...
private:
	...
	// Connection point on InkOverlay
