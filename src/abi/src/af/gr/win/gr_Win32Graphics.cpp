...
GR_Win32Graphics::GR_Win32Graphics(HDC hdc, HWND hwnd, XAP_App * app, GR_Win32InkOverlay * pInkOverlay)
{
	...
	// KMC: ink
	m_pInkOverlay = pInkOverlay;
}

GR_Win32Graphics::GR_Win32Graphics(HDC hdc, const DOCINFO * pDocInfo, XAP_App * app, GR_Win32InkOverlay * pInkOverlay)
{
	_constructorCommonCode(hdc);	
	m_pApp = app;
	m_bPrint = true;
...
void GR_Win32Graphics::scroll(UT_sint32 dx, UT_sint32 dy)
{
	...
	this->getInkOverlay()->scrollInk(this, -dx, -dy);

	ScrollWindowEx(m_hwnd, -dx, -dy, NULL, NULL, NULL, 0, SW_INVALIDATE);

	// hack for ink scrolling problems
	RedrawWindow(m_hwnd, NULL, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_ERASENOW); 
}
							  ...
							  UT_sint32 width, UT_sint32 height)
{
	...
	this->getInkOverlay()->scrollInk(this, x_dest - x_src, y_dest - y_src);
...
// KMC
// returns bool signifying if we have a valid ink overlay or not
bool GR_Win32Graphics::hasInkOverlay() {
	bool hasInk = false;

	if(this->m_pInkOverlay != 0) {
		hasInk = true;
	}
...
// KMC
// returns a pointer to the current GR_Win32InkOverlay
GR_Win32InkOverlay * GR_Win32Graphics::getInkOverlay() {
	return this->m_pInkOverlay;
}

// KMC
// refresh the screen (aka get the ink to redraw)
void GR_Win32Graphics::refreshScreen() {
