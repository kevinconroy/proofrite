...
EV_EditBinding * EV_EditBindingMap::findEditBinding(EV_EditBits eb)
{
  ...
  // do the tablet check first since we're going to piggy back on the mouse code - out of order
  //  call would cause ALL tablet actions to be interpreted as strokes (see ev_EditBits.h)
  /*
  if (EV_IsTablet(eb))
  {		
		class ev_EB_TableTable * p = m_pebTB;
		if (!p)
			return 0;					// no bindings of anykind for this mouse button
		UT_uint32 n_etb = EV_ETB_ToNumber(eb);
		UT_uint32 n_ems = EV_EMS_ToNumber(eb);
		return p->m_peb[n_etb][n_ems];
  }
  else */
