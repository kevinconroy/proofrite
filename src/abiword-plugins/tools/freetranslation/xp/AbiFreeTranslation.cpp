...
// 
inline static char * _ucsToLatin1(const UT_UCSChar * text)
{
	// calculate length of text so that we can create a character
	// buffer of equal size.
	const unsigned int length = UT_UCS4_strlen((const UT_UCS4Char *) text);
...
static bool _getTranslationCode(FV_View * pView, UT_String & langCode)
{
	...
	XAP_Dialog_Language * pDialog = (XAP_Dialog_Language *)(pDialogFactory->requestDialog(id));
	UT_ASSERT(pDialog);
		...
		if (_getTranslationCode(pView, langCode))
	   	{
			...
			char * translate = _ucsToLatin1(pView->getSelectionText());
			...
			url = "http://fets3.freetranslation.com:5081/?Sequence=core";
			url += "&Language=";
			url += langCode;
			url += "&SrcText=";
			url += srcText;
