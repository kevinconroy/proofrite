...
dnl= abi_plugin_macro="ABIPGN_ABIGDKPIXBUF"
dnl= 
...
AC_DEFUN([ABIPGN_ABIGDKPIXBUF],[

_abi_cppflags_save="$CPPFLAGS"
_abi_ldflags_save="$LDFLAGS"

ABIGDKPIXBUF_CPPFLAGS=""
ABIGDKPIXBUF_LDFLAGS=""

# Checks for libraries.
# Checks for header files.

PKG_CHECK_MODULES(_abi_gdk_pixbuf,gdk-pixbuf-2.0 >= 2.0,[
	ABI_PLUGIN_REPORT([AbiGdkPixbuf: okay])
],[	ABI_PLUGIN_REPORT([AbiGdkPixbuf: error - gdk-pixbuf-2.0 >= 2.0 required])
	abi_plugin_disable=yes
])
if test $abi_plugin_disable = no; then
	ABIGDKPIXBUF_CPPFLAGS="$_abi_gdk_pixbuf_CFLAGS"
	ABIGDKPIXBUF_LDFLAGS="$_abi_gdk_pixbuf_LIBS"
fi

AC_SUBST(ABIGDKPIXBUF_CPPFLAGS)
AC_SUBST(ABIGDKPIXBUF_LDFLAGS)
