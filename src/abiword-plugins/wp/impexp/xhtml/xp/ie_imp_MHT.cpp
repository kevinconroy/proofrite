...
{
	UT_uint32 needle_length = (UT_uint32) strlen (needle);
	UT_uint32 i = 0;
...
UT_Error IE_Imp_MHT::importFile (const char * szFilename)
{
	...
	for (struct header_t * h = eps_next_header (m_eps); h; h = eps_next_header (m_eps))
		{
			const char * name = (const char *) h->name;
			const char * data = (const char *) h->data;

			if (name && data)
				if (UT_stricmp (name, "content-type") == 0)
					{
						UT_uint32 length = (UT_uint32) strlen (data);
	...
	for (UT_uint32 i = 0; i < count; i++)
		{
			...
			if (bContentID)
				{
					if (ptr->contentID ())
						if (strncmp ((const char *) szSrc + 4, ptr->contentID () + 1, strlen ((const char *) szSrc) - 4) == 0)
							{
								part = ptr;
								break;
							}
			...
			else
				{
					if (ptr->contentLocation ())
						if (strcmp ((const char *) szSrc, ptr->contentLocation ()) == 0)
							{
								part = ptr;
								break;
							}
	...
	for (unsigned char * l = mime_next_line (m_eps); l; l = mime_next_line (m_eps))
		{
			...
			UT_uint32 length = (UT_uint32) strlen (line);
...
bool UT_Multipart::append (const char * buffer, UT_uint32 length)
{
	...
	return (m_buf->append ((const UT_Byte *) buffer, length) && m_buf->append ((const UT_Byte *) s_newline, 1));
}
			...
			if (bEnd || (m_b64length == 80) || ((i + 1 == length) && m_b64length && ((m_b64length & 0x03) == 0)))
				{
					...
					if (binlength < 60)
						if (!m_buf->append ((const UT_Byte *) binbuffer, 60 - binlength)) success = false;
				}
...
bool UT_Multipart::append_Quoted (const char * buffer, UT_uint32 length)
{
	...
	*strptr = 0;
	...
	bool success = m_buf->append ((const UT_Byte *) str, strlen (str));
