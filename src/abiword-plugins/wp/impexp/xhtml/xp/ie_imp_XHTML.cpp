...
UT_Confidence_t IE_Imp_Tidy_Sniffer::recognizeContents(const char * szBuf, 
											 UT_uint32 iNumbytes)
{
	return UT_CONFIDENCE_ZILCH;
}
...
UT_Error IE_Imp_Tidy_Sniffer::constructImporter(PD_Document * pDocument,
												 IE_Imp ** ppie)
{
	IE_Imp_XHTML * p = new IE_Imp_XHTML(pDocument);
	if (p) p->useTidy ();
...
#ifdef XHTML_HTML_SUPPORTED
	TidyReader wrapper;
	if (m_bUseTidy) setReader (&wrapper);
#endif
