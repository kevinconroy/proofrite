...
IE_Exp_WordPerfect_Sniffer::IE_Exp_WordPerfect_Sniffer (const char * name) :
  IE_ExpSniffer(name)
{
  // 
}
...
UT_Error IE_Exp_WordPerfect::_writeHeader()
{
	...
	*m_buffer += (char)0x00;						// reserved; completeness indicator
	...
	for (int i=0; i<488; i++) { *m_buffer += (char)0x00; } // extended header, 488 bytes
	...
	*m_buffer += (char)0x00;						// index header; reserved = 0
	...
	for (int i=0; i<10; i++) { *m_buffer += (char)0x00; } // index header; reserved = 0 x 10
