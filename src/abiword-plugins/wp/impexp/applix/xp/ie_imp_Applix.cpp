...
void IE_Imp_Applix::_applixDecodeText (const char *buf, size_t len)
{
	...
	do 
	{
		...
		case '^':
			...
			if (ch != '^')
			{
				...
				m_textBuf.append((const UT_GrowBufElement *)&c, 1);
				ch = 0;
			}
			break;
		}
	...
	{
		getDoc()->appendSpan ((const UT_UCSChar *)m_textBuf.getPointer (0), m_textBuf.getLength ());
		m_textBuf.truncate(0);
	}
}
...
void IE_Imp_Applix::_applixNewPara (const char *buf, size_t len)
{
	...
	if (runlen > 0) 
	{
		...
		getDoc()->appendSpan ((const UT_UCSChar *)m_textBuf.getPointer (0), runlen);
	}
