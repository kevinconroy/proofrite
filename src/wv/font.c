...
void
wvGetFONTSIGNATURE (wvFONTSIGNATURE * fs, wvStream * fd)
{
    int i;
    for (i = 0; i < 4; i++)
...
void
wvInitFONTSIGNATURE (wvFONTSIGNATURE * fs)
{
    int i;
    for (i = 0; i < 4; i++)
	fs->fsUsb[i] = 0;
...
void
wvGetPANOSE (wvPANOSE * item, wvStream * fd)
{
